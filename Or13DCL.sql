...
사용자권한
설명 : 새로운 사용자계정을 생성하고 시스템권한을
      부여하는 방법을 학습
******************************/

/*
[사용자 계정 생성 및 권한설정]
해당 부분은 DBA권한이 있는 최고관리자(sys, system)
으로 접속한 후 실행해야 한다.
...
*/
/* 오라클 12C 이후부터는 일반 계정을 생성할 때 접두어로 C##
을 써야지만 계정이 정상적으로 생성된다.*/
create user c##test_user1 identified by 1234;

/* 하지만 계정에 C##을 붙이는것은 조금 불편하므로 아래와 같이
session을 변경하는 명령을 실행한 후 계정을 생성하면 접두어 없이
생성할 수 있다. */
...
/*
    create table 권한 부여 후 테이블 생성 및
    desc명령으로 스키마를 확인할 수 있다.
*/
...
/*
    exit 혹은 quit 명령을 통해 접속을 해제한 후 다시 접속하면
    기존의 암호로는 접속되지 않는다. 변경한 암호로 접속해야 한다.
*/
...
4] Role(롤, 역할)을 통해 여러가지 권한을 동시에 부여하기
    : 여러 사용자가 다양한 권한을 효과적으로 관리할 수 있도록 관련된
      권한끼리 묶어놓은것을 말한다.
※ 우리는 실습을 위해 새롭게 생성한 계정에 connect, resource롤을 
주로 부여한다. 
*/
...
drop role my_role;
/*
    test_user3은 my_role을 통해 권한을 부여받았으므로 해당
    Role을 삭제하면 부여받았던 모든 권한이 회수(Revoke) 된다.
    즉, Role삭제 후에는 접속할 수 없다.
*/
...
/*
    접속권한 회수 후 접속을 시도하면 비밀번호가 틀린경우에는
    '부적합'에러가 발생하고, 비밀번호가 일치하면 create session
    권한이 없다고 나온다.
*/

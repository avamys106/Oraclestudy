...
시나리오] 앞에서 if~elsif로 작성한 PL/SQL문을 
        case~when문으로 변경하시오.
*/
declare
    emp_id employees.employee_id%type := &emp_id;
...
declare
    ...
    변수명을 사용하지만, Oracle 에서는 그룹함수 sum()이 있으므로
...
/*
예제14] 제어문(반복문 : while) 
시나리오] while loop문으로 1부터 10까지의
        정수의 합을 구하는 프로그램을 작성하시오. 
*/
...
반복문3 : for문
    ...
    변수를 별도로 선언하지 않아도된다. 그러므로 특별한 이유가 없다면
...
declare
begin
    for num4 in 2 .. 9 loop
        for num5 in 1 .. 9 loop
            dbms_output.put_line(num4 || '*' || num5 || '=' || num4*num5);
        end loop;
    end loop;
end;
/

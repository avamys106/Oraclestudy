...
시나리오] 앞에서 생성한 뷰를 다음과 같이 수정하시오.
    기존 컬럼인 employee_id, first_name, job_id, 
    hire_date, department_id를
...
select * from view_employees;
/*
    뷰 생성시 기존테이블의 컬럼명을 변경해서 출력하고 싶다면 위와같이
...
as
    select 
        employee_id e_id, concat(first_name||' ' ,last_name) name, email, manager_id m_id
    from employees where job_id ='ST_MAN';
--뷰 확인
...
from employees;
--뷰로 생성
/* 생성시 연산식이 추가되어 논리적으로 컬럼이 추가되는 경우에는 반드시
별칭으로 컬럼명을 명시해야 한다. 그렇지 않으면 에러가 발생한다. */

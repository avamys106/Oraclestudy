...
select
    ...
    trim(both '다' from '다람쥐가 나무를 탑니다') trim3, /* both는 디폴트값이므로 trim2와 결과 동일 */
    trim(leading '다' from '다람쥐가 나무를 탑니다') trim4, /* 좌측의 문자 제거 */
    trim(trailing '다' from '다람쥐가 나무를 탑니다') trim5, /* 우측의 문자 제거 */
    trim('다' from '다람쥐가 다리위의 나무를 탑니다') trim6 /* 중간의 문자는 제거할 수 없다. */
from dual;
...
substr() : 문자열에서 시작인덱스부터 길이만큼 잘라서 문자열을 출력한다.
    ...
    참고3) 시작인덱스가 음수면 우측끝부터 좌로 인덳스를 적용한다.
*/

select substr('good morning john', 9, 4) from dual;--ning
select substr('good morning john', 9) from dual;--n부터 끝까지 출력
...
통해 *를 출력하면 첫글자를 제외한 남은 길이만큼을 마스킹 처리할 수 있다. */
select first_name,
...
replace() : 문자열을 다른 문자열로 대체할때 사용한다. 만약 공백으로
    ...
    ※trim(), ltrim(), rtrim()의 기능을 replace()메소드 하나로

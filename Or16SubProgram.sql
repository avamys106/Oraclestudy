...
서브프로그램
설명 : 저장프로시저, 함수 그리고 프로시져의 일종인 트리거를 학습
**************************/
...
*/
/*프로시저 생성시 in파라미터를 설정한다. first_name을 
  참조하는 참조변수로 선언한다.*/
...
/*
update 기능이 있는 프로시저 호출을 위해 employees 테이블을 레코드까지
복사한 후 학습을 진행한다. 
*/
create table zcopy_employees
as
select * from employees where 1=1;
/* select절에 where을 생략하거나 1=1의 조건을 주면 참이 되므로
모든 레코드를 함께 복사한다. 만약 스키마(구조)만 복사하고 싶다면
1=0과 같이 부정의 조건을 기술하면된다. */
...
*/
/*
in파라미터는 사원번호와 수정할 급여를 입력받는다. out파라미터는
update가 적용된 행의 갯수를 반환한다. 
*/
...
execute pcd_update_salary(100, 5000, :r_count);
...
select * from zcopy_employees where employee_id=777;

/*
2.함수
-사용자가 PL/SQL문을 사용하여 오라클에서 제공하는 내장함수와 같은
기능을 정의한것이다.
-함수는 in파라미터만 사용할 수 있고, 반드시 반환값의 자료형을
명시해야한다.
-프로시저는 여러개의 결과값을 얻어올 수 있지만, 함수는 반드시
하나의 값을 반환해야한다.
-함수는 쿼리문의 일부분으로 사용된다.
...
/*
시나리오] 2개의 정수를 전달받아서 두 정수사이의 
모든수를 더해서 결과를 반환하는 함수를 정의하시오.
실행예) 2, 7 -> 2+3+4+5+6+7 = ??
*/
...
return
    ...
    varchar2 
is
    /* 반환값을 저장할 변수로 부서테이블의 부서명 컬럼을 참조하는 
    변수로 설정 */
...
3.트리거(Trigger)
    : 자동으로 실행되는 프로시저로 직접 실행은 불가능하다.
      주로 테이블의 입력된 레코드의 변화가 있을때 자동으로 실행된다.
*/
...
create or replace trigger trig_dept_backup
    ...
    after
    /* 이벤트 : insert, update, delete와 같은 
               쿼리 실행시 발생된다.*/
    INSERT
    --트리거를 적용할 테이블명
    on trigger_dept_original
    /* 행단위 트리거를 정의한다. 즉 하나의 행이 변화할때마다
    트리거가 수행된다. 만약 문장(테이블)단위 트리거로 정의하고
    싶다면, 해당 문장을 생략하면된다. 이 경우에는 쿼리를 한번
    실행하면 트리거도 딱 한번만 실행된다. */
    for each row
begin
    if Inserting then
...
insert into trigger_dept_original values (201, '전산팀', 20, 100);
insert into trigger_dept_original values (301, '행정팀', 30, 100);
--각 테이블의 레코드 확인
select * from trigger_dept_original;
select * from trigger_dept_backup;
...
delete from trigger_dept_original where department_id=201;
...
/*
생성2 : 오리지널 테이블에 업데이트 이후 테이블(문장) 단위로 발생하는
    트리거 생성
*/
create or replace trigger trigger_update_test
    after update on trigger_dept_original

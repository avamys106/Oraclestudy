...
시퀀스 생성시 주의사항
1.start with에 minvalue보다 작은값을 지정할 수 없다.
  즉 start with값은 minvalue와 같거나 커야한다.
2.nocycle로 설정하고 시퀀스를 계속 얻어올때 maxvalue에
  지정값을 초과하면 에러가 발생한다.
...
/* 데이터 사전에서 확인하기(결과를 보면 PK 혹은 Unique로
지정된 컬럼은 자동으로 인덱스가 생성되어 잇는것을 볼 수 있다.) */
select * from user_ind_columns;

--인덱스 삭제
drop index tb_goods_name_idx;

--인덱스 수정은 불가능하다. 필요하다면 삭제 후 다시 생성해야한다.

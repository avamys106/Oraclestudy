...
아니므로 주의해야 한다. */
select to_char(sysdate, '"오늘은 "YYYY"년 "MM"월 "DD"일 입니다"') "DATE" from dual;
select to_char(sysdate, '"오늘은 "YYYY"년 "MM"월 "DD"일" DAY" 입니다."') "DATE" from dual;
/*
시나리오] 사원테이블에서 사원의 입사일을 
*/
--서식문자 day는 "월요일", dy는 "월"만 출력해준다.
select first_name, hire_date, to_char(hire_date, 'YYYY"년 "MM"월 "DD"일" DAY" 입니다."')"날짜"
from employees;
...
select
    to_number('123,000')+to_number('10')
    from dual;
--replace()로 콤마를 제거한 후 숫자로 변환하고 덧셈을 진행한다.
select
    to_number(replace('123,000',',',''))+'10'
    from dual;
...
select
to_date('2021년01월01일', 'yyyy"년"mm"월"dd"일"')"날짜",
    to_char(to_date('2021년01월01일', 'yyyy"년"mm"월"dd"일"'),'DAY')"요일"
from dual;
select
...
select ename, substr(hiredate,1,5)"HIREDATE" from emp;
...
select ename, to_char(to_date(hiredate, 'YY-MM-DD')
,'YY MON DY') "HIREDATE" from emp;

/* 5. 올해 며칠이 지났는지 출력하시오. 현재 날짜에서 올해 1월1일을 뺀 결과를 출력하고
TO_DATE()함수를 사용하여 데이터 형을 일치 시키시오. 단, 날짜의 형태는 ‘01-01-2020’ 
포맷으로 사용한다. 즉 sysdate - ‘01-01-2020’ 이와같은 연산이 가능해야한다.*/
--에러발생
select to_date('01-01-2023') from dual;
--서식문자를 통해 날짜를 인식시킴
select to_date('01-01-2023','dd-mm-yyyy') from dual;

select trunc(sysdate - to_date('01-01-2024','dd-mm-yyyy')) from dual;

/* 6. 사원들의 메니져 사번을 출력하되 매니져가 없는 사원에 대해서는
NULL값 대신 0으로 출력하시오. */
select 
    empno, ename, nvl(mgr)
from emp;

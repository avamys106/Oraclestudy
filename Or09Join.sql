...
select
    ...
    /*이부분에서 에러발생*/department_id, department_name
...
--5.join 쿼리문 작성
/* 양쪽 테이블에 동시에 존재하는 컬럼인 경우에는 반드시 테이블명이
별첨을 명시해야 한다. */
select 
...
--테이블의 별칭을 사용하면 쿼리문을 조금 간단하게 만들 수 있다.
select 
    first_name, last_name, email, 
...
select 
    employee_id, first_name, last_name, Emp.department_id,
...
from employees Emp left outer join departments Dep
    on Emp.department_id = Dep.department_id where hire_date like '07%';

select 
 employee_id, first_name, last_name, nvl(department_name,'<부서없음>'), hire_date
from employees Emp, departments Dep
where
    Emp.department_id=Dep.department_id(+) and hire_date like '07%';
...
select
    ...
    concat(concat(empManager.first_name,' '), empManager.last_name) "메니져이름"
...
select
    employee_id, first_name, last_name, trim(to_char(salary, '$999,000')) department_name,

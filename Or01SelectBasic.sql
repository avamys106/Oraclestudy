...
설명 : select, where 등 가장 기본적은 DQL문 사용해보기
******/
...
select first_name, salary, commission_pct,
    salary+(salary*commission_pct) as SalComm from employees;
--as는 생략할 수 있다.
...
--first_name이 세번째문자인 a인 직원을 추출하시오.
...
--물리적으로 존재하는 컬럼으로 정렬
select ename, sal ,sal*12+100 as AnnualSalary 
from emp order by sal desc;
--AS를 통해 부여한 별칭으로 정렬
select ename, sal ,sal*12+100 as AnnualSalary 
from emp order by AnnualSalary desc;
...
--select *from emp where empno like '%7782%'; 
select ename, deptno 
from emp where empno like '%7782%'; 
...
/* 6. 입사일이 81년2월20일 부터 81년5월1일 사이인
...
from emp where deptno=20 or deptno=30 order by ename desc;

/* 8. 사원의 급여가 2000에서 3000사이에 포함되고 부서번호가 20 또는 30인 사원의 이름,
급여와 부서번호를 출력하되 이름순(오름차순)으로 출력하시오 */
select ename, sal, deptno 
from emp where (sal >= 2000 and sal <= 3000) and (deptno=20 or deptno =30)
order by ename asc;
...
select ename
from emp where instr(ename, 'R')=3;
/* select ename
...
select ename
from emp where ename like '%A%E%';

/* 14. 담당업무가 사무원(CLERK) 또는 영업사원(SALESMAN)이면서
급여가 $1600, $950, $1300 이 아닌 사원의 이름, 담당업무, 급여를 출력하시오. */
select ename, job, sal
from emp where (job ='CLERK' or job ='SALESMAN') and not sal in(1600, 950, 1300);
